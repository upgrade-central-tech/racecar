    ...
    std::span<uint32_t> indices, std::span<Vertex> vertices )
{
    ...
    // index buffer data on GPU. Use vkCmdCopyBuffer to transfer data from CPU to GPU
    ...
    log::info( "Uploaded mesh (vert+idx buffers) to GPU" );
...
void generate_tangents( Mesh& mesh )
{
    ...
    for ( uint32_t index = 0; index < static_cast<uint32_t>( indices.size() ); index += 3 ) {
        ...
        float w = glm::dot( glm::normalize( glm::cross( vertices[i0].normal, tangent ) ),
