    ...
    const State& engine, PipelineBarrierDescriptor barrier, VkCommandBuffer cmd_buf )
{
    ...
    std::transform( barrier.buffer_barriers.begin(), barrier.buffer_barriers.end(),
        std::back_inserter( vk_buffer_barriers ), []( BufferBarrier b ) { return b.get_vk(); } );

    std::transform( barrier.image_barriers.begin(), barrier.image_barriers.end(),
