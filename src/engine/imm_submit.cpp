...
)
{
    ...
    vk::check(
        vkResetCommandBuffer( immediate_submit.cmd_buf, 0 ),
    ...
    vk::check(
        vkQueueSubmit2( vulkan.graphics_queue, 1, &submit, immediate_submit.fence ),
...
void create_immediate_sync_structures( ImmediateSubmit& immediate_submit )
{
    ...
    vk::check(
        vkCreateFence( vulkan.device, &fence_info, nullptr, &immediate_submit.fence ),
