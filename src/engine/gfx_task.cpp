...
bool execute_gfx_task( const vk::Common& vulkan,
    ...
    for ( size_t i = 0; i < task.draw_tasks.size(); i++ ) {
        draw( vulkan, engine, task.draw_tasks[i], task.command_buffer );
    }
...
bool draw( const vk::Common& vulkan,
           const engine::State& engine,
           const DrawTaskDescriptor& draw_task,
