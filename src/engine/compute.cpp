...
void execute_compute_task(const engine::State& engine, const VkCommandBuffer& cmd_buf, ComputeTask &compute_task) {
    ...
    for ( size_t i = 0; i < compute_task.descriptor_sets.size(); ++i ) {
        DescriptorSet* descriptor_set = compute_task.descriptor_sets[i];
