...
void execute_cs_task( const VkCommandBuffer& cmd_buf, ComputeTask& compute_task )
{
    ...
    vkCmdDispatch(
        cmd_buf,
        uint32_t( compute_task.group_size.x ),
        uint32_t( compute_task.group_size.y ),
        uint32_t( compute_task.group_size.z )
    );
}
