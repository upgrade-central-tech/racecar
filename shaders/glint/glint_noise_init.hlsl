...
static const uint2 FRAME_SIZE = uint2(512, 512);

uint CoordToFlatId(uint2 coord)
{
	return coord.y * FRAME_SIZE.x + coord.x;
}
...
[noinline]
float4 sample_packed_noise(float2 uv) {
    // Hardcoded for fast testing
    uint seed = 1u;
    uint2 size = FRAME_SIZE.xy;

    // Generate random numbers for this cell and the next ones in X and Y
    int2 pixelCoord00 = int2(uv * FRAME_SIZE.xy);
    uint rngState00 = WangHash(CoordToFlatId(pixelCoord00 * 123) + FRAME_SIZE.x * FRAME_SIZE.y * seed);
    ...
    uint rngState01 = WangHash(CoordToFlatId(pixelCoord01 * 123) + FRAME_SIZE.x * FRAME_SIZE.y * seed);
    ...
    uint rngState10 = WangHash(CoordToFlatId(pixelCoord10 * 123) + FRAME_SIZE.x * FRAME_SIZE.y * seed);
    ...
    uint rngState11 = WangHash(CoordToFlatId(pixelCoord11 * 123) + FRAME_SIZE.x * FRAME_SIZE.y * seed);
